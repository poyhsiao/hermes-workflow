...
_WF_STEP_KNOWN_FIELDS: frozenset[str] = frozenset(
    {
        ...
        "event_name",
        "tool",
        "command",  # legacy YAML step-level fields (silently ignored by parser)
        "agent_goal",
        "agent_profile",  # top-level agent step fields
        "branches",  # parallel_branch sub-field
        "profile",
        "goal",  # agent sub-fields (when agent is flat dict, not nested)
    }
)
...
def validate_workflow(data: dict) -> None:
    ...
    if "steps" in data and isinstance(data["steps"], list):
        ...
        for i, step in enumerate(data["steps"]):
            ...
            unknown_step = sorted(set(step.keys()) - _WF_STEP_KNOWN_FIELDS)
            if unknown_step:
